"""HTTP routes for encounters-service.

Generic CRUD over the `encounters` table. Every mutating endpoint publishes a
domain event (encounter.started) and an audit event; reads publish audit only.
...
TABLE = "encounters"
RESOURCE = "encounters"
CREATED_EVENT = "encounter.started"
UPDATED_EVENT = "encounter.ended"
...
def build_blueprint(svc) -> Blueprint:
    ...
    def create_record():
        ...
        if payload.get('provider_id'):
            _peer = clients.get('providers-service')
            if _peer:
                try:
                    payload['provider_snapshot'] = json_or_raise(
                        _peer.get(f"/api/providers/" + str(payload['provider_id']))
                    )
                except ServiceUnavailable as _e:
                    payload['provider_warn'] = str(_e)
        row = db.query_one(
